...
class CodeBERT(nn.Module):
    ...
    def get_results(self, dataset, batch_size, output_attentions=True):
...
class GraphCodeBERT(nn.Module):
    ...
    def forward(self, inputs_ids=None, attn_mask=None, position_idx=None, labels=None, output_attentions=True):
        ...
        outputs = self.encoder.roberta(inputs_embeds=inputs_embeddings, attention_mask=attn_mask,
                                       position_ids=position_idx, output_attentions=output_attentions)
        sequence_outputs = outputs[0]
        logits = self.classifier(sequence_outputs)
        prob = F.softmax(logits)
    ...
    def get_results(self, dataset, batch_size, output_attentions=True):
        ...
        for batch in eval_dataloader:
            ...
            with torch.no_grad():
                lm_loss, logit, attention = self.forward(inputs_ids, attn_mask, position_idx, label, output_attentions)
                logits.append(logit.cpu().numpy())
                attentions.append(attention)
