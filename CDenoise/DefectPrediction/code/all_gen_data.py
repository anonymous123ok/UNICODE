...
def generate(args, model, tokenizer, mlm_tokenizer, data_file, method, attack_method):
    ...
    if not os.path.exists(w_path):
        os.makedirs(w_path)
    open(w_path + '/mlm_%s_%s_all.txt' % (args.model_name, method), 'w').writelines(results)
...
def main():
    ...
    # output_dir = "DefectPrediction/code/saved_models/model.bin"
    ...
    for attack_method in attack_methods:
        ...
        path1 = "adv_train/defec/%s/%s" % (attack_method, args.model_name)
        args.train_data_file = path1 + '/train.txt'
        args.eval_data_file = '../dataset_old/valid.txt'
