...
from utils import CodeDataset, GraphCodeDataset, CodeT5Dataset, is_valid_variable_name, output_weights,GraphCodeDataset_clone
...
def is_incorrect(args, original_model, original_tokenizer, all_vars, identifiers, ori_codes, ori_labels, pred_labels):
    ...
    for i in range(args.N):
        for iden in identifiers:
            ...
            while len(gen_idens) < args.theta and count < len(random_index):
            ...
            code1 = ori_codes.split("</s><s> ")[0].split("<s>")[-1]
            code2 = ori_codes.split("</s><s> ")[-1].split("</s>")[0]

            for gen_iden in gen_idens:
                temp_code = get_example(code1, iden, gen_iden, args.language_type)
                replace_idens.append(gen_iden)
                if args.model_name == 'codebert':
                    new_feature = codebert_convert_examples_to_features(temp_code,code2,ori_labels, original_tokenizer, args)
                elif args.model_name == 'graphcodebert':
                    new_feature = graphcodebert_convert_examples_to_features(temp_code,code2, ori_labels, original_tokenizer, args)
                elif args.model_name == 'codet5' or args.model_name == 'codet5p':
                    new_feature = codet5_convert_examples_to_features(temp_code,code2, ori_labels, original_tokenizer, args)
                replace_examples.append(new_feature)
    if args.model_name == 'codebert':
        new_dataset = CodeDataset(replace_examples)
    elif args.model_name == 'graphcodebert':
        new_dataset = GraphCodeDataset_clone(replace_examples, args)
    elif args.model_name == 'codet5' or args.model_name == 'codet5p':
...
def evaluate(args, mlm_model, mlm_tokenizer, original_model, original_tokenizer, eval_when_training=False):
    ...
    all_vars = [i.strip() for i in open('../dataset/All_vars/%s/%s/all_vars.txt' % (args.attack,args.model_name), 'r').readlines()]
    ...
    for batch_i, batch in enumerate(tqdm(eval_dataloader)):
        ...
        try:
            if is_incorrect(args, original_model, original_tokenizer, all_vars, identifiers, ori_codes, ori_labels,
                        pred_labels) == 0:
                if ori_labels == pred_labels:
                    not_clean_but_right += 1
                continue
        except:
            continue
        ...
        # print("mask_codes : ", mask_codes)
        m_code1 = mask_codes.split("</s><s>")[0].split("<s>")[-1]
        m_code2 = mask_codes.split("</s><s>")[-1].split("</s>")[0]

        # print("m_code1 : ",m_code1)
        # print("\n\n\n\n\n\nm_code2 : ",m_code2)

        if args.model_name == 'codebert':
            new_feature = codebert_convert_examples_to_features(m_code1,m_code2, ori_labels,original_tokenizer,args)
        elif args.model_name == 'graphcodebert':
            new_feature = graphcodebert_convert_examples_to_features(m_code1,m_code2, ori_labels,original_tokenizer,args)
        elif args.model_name == 'codet5' or args.model_name == 'codet5p':
            new_feature = codet5_convert_examples_to_features(m_code1,m_code2, ori_labels,original_tokenizer,args)
        if args.model_name == 'codebert':
            new_dataset = CodeDataset([new_feature])
        elif args.model_name == 'graphcodebert':
            new_dataset = GraphCodeDataset_clone([new_feature], args)
        elif args.model_name == 'codet5' or args.model_name == 'codet5p':
        ...
        # print("new_feature.input_tokens : ", new_feature.input_tokens)
        # print("identifiers :",identifiers)
        if args.model_name == 'graphcodebert':
            attention_weights = output_weights(attentions, new_feature.input_tokens_1, identifiers)
        else:
        ...
        for iden in identifiers:
            ...
            temp_mask_codes = get_example(m_code1, iden, '<mask>', args.language_type)
            ...
            replace_examples = []
            replace_idens = []
            ori_code1 = ori_codes.split("</s><s> ")[0].split("<s>")[-1]
            ori_code2 = ori_codes.split("</s><s> ")[-1].split("</s>")[0]
            for gen_iden in gen_idens:
                temp_code = get_example(ori_code1, iden, gen_iden, args.language_type)
                replace_idens.append(gen_iden)
                if args.model_name == 'codebert':
                    new_feature = codebert_convert_examples_to_features(temp_code, ori_code2, ori_labels, original_tokenizer,args)
                elif args.model_name == 'graphcodebert':
                    new_feature = graphcodebert_convert_examples_to_features(temp_code, ori_code2, ori_labels, original_tokenizer,args)
                elif args.model_name == 'codet5' or args.model_name == 'codet5p':
                    new_feature = codet5_convert_examples_to_features(temp_code, ori_code2, ori_labels, original_tokenizer,args)
                replace_examples.append(new_feature)
            if args.model_name == 'codebert':
                new_dataset = CodeDataset(replace_examples)
            elif args.model_name == 'graphcodebert':
    ...
    # ans = results['repair']/total
    ans = (results['repair'] + not_clean_but_right) / total
    print('  Repair:', results['repair'])
    print('  Incorrect:', results['incorrect'])
    #print('  Acc:', str(round(results['acc'] * 100, 2)) + '%')
    print("not_clean_but_right = ", not_clean_but_right)
    print('ans = ', ans)
    return results
...
def main():
    ...
    parser.add_argument("--attack", default=None, type=str, required=True, help="attack.")
    ...
    original_model.load_state_dict(torch.load('./%s/checkpoint-best-f1/%s_model.bin' % (args.output_dir, args.model_name)))
    ...
    print('Load mlm Model ...')
    if args.model_name == 'codet5p':
        aaa = 'codet5'
    else:
        aaa = args.model_name
    if os.path.exists('./%s/mlm/%s_model.bin' % (args.output_dir, aaa)):
        mlm_model.load_state_dict(torch.load('./%s/mlm/%s_model.bin' % (args.output_dir, aaa)))
        # mlm_model.load_state_dict(torch.load("../Finetune_mlm/mlm_model.bin"))
    mlm_model.to(args.device)

    args.train_data_file = '../dataset/mlm_%s_train.txt' % (args.model_name)
    # args.eval_data_file = (
    #         '../dataset/All_vars/%s/%s/mlm_%s_test_all.txt' % (
    # args.attack, args.model_name, args.model_name))
    args.eval_data_file = "../dataset/test_.txt"
