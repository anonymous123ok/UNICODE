...
class GraphCodeBertInputFeatures(object):
    ...
    def __init__(self, input_tokens_1, input_ids_1, position_idx_1, dfg_to_code_1, dfg_to_dfg_1, input_tokens_2, input_ids_2, position_idx_2, dfg_to_code_2, dfg_to_dfg_2, label):
        ...
        try:
            ...
            with open(file_path) as f:
                for line in f:
                    ...
                    self.examples.append(codebert_convert_examples_to_features(code1,code2, int(label), tokenizer, args))
        ...
        try:
            ...
            with open(file_path) as f:
                for idx, line in enumerate(f):
                    ...
                    code2 = idx2jsonl[url2]
                    content = '<s>' + code1 + '</s><s>' + code2 + '</s>'.replace("\\n", "\n").replace('\"', '"')
                    self.examples.append(graphcodebert_convert_examples_to_features(code1,code2, int(label), tokenizer, args))
                    self.code_files.append(content)
            assert (len(self.examples) == len(self.code_files))
            with open(code_pairs_file_path, 'wb') as f:
                pickle.dump(self.code_files, f)
        ...
        try:
            ...
            with open(file_path) as f:
                for line in f:
                    ...
                    code2 = idx2jsonl[url2]
                    content = '<s>' + code1 + '</s><s>' + code2 + '</s>'.replace("\\n", "\n").replace('\"', '"')
                    self.examples.append(codet5_convert_examples_to_features(code1,code2, int(label), tokenizer, args))
                    self.code_files.append(content)
            assert (len(self.examples) == len(self.code_files))
            with open(code_pairs_file_path, 'wb') as f:
                pickle.dump(self.code_files, f)
