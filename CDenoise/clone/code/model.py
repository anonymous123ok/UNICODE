    ...
    def forward(self, input_ids=None, labels=None, output_attentions=True):
        ...
        if labels is not None:
            ...
            if output_attentions:
                return loss, prob, opt.attentions
            else:
                return loss, prob, None
        else:
            return prob

    def get_results(self, dataset, batch_size, output_attentions=True, threshold=0.5):
        ...
        for batch in eval_dataloader:
            ...
            with torch.no_grad():
                lm_loss, logit, attention = self.forward(inputs, label, output_attentions)
        ...
        return probs, pred_labels, attentions
...
class GraphCodeBERT(nn.Module):
    ...
    def forward(self, inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2, position_idx_2, attn_mask_2,
                labels=None, output_attentions=True):
        ...
        opt = self.encoder.roberta(inputs_embeds=inputs_embeddings, attention_mask=attn_mask, position_ids=position_idx,
    ...
    def get_results(self, dataset, batch_size, output_attentions=True, threshold=0.5):
        ...
        for batch in eval_dataloader:
            ...
            with torch.no_grad():
                lm_loss, logit, attention = self.forward(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
                                                         position_idx_2,
                                                         attn_mask_2, label,output_attentions=output_attentions)
                logits.append(logit.cpu().numpy())
                attentions.append(attention)
...
class CodeT5(nn.Module):
    ...
    def forward(self, input_ids=None, labels=None, output_attentions=True):
        ...
        if labels is not None:
            ...
            loss = loss_fct(logits, labels)
            if output_attentions:
                return loss, prob, opt.encoder_attentions
            else:
                return loss, prob, None
        else:
            return prob
