...
def main():
    ...
    if args.model_name == 'codebert':
        ...
        all_examples = CodeT5TextDataset(tokenizer, args, args.all_data_file)
    ...
    with open(args.store_path, "w") as wf:
        for index in tqdm(range(len(all_data))):
            ...
            if not true_label == orig_label:  # true_label是真正的标签  orig_label是模型推理的结果
                continue
            ...
            if not os.path.exists('../dataset/%s_all_subs' % args.model_name):
                os.makedirs('../dataset/%s_all_subs' % args.model_name)
            np.save('../dataset/%s_all_subs/%s_%s_%s' % (args.model_name, str(orig_label), str(index), '1'),
                    embeddings1.cpu().numpy())
            np.save('../dataset/%s_all_subs/%s_%s_%s' % (args.model_name, str(orig_label), str(index), '2'),
