...
def get_example(item):
    shilihua_code1, url1, url2, label, tokenizer, args, cache, url_to_code = item

    try:
        code = ' '.join(shilihua_code1.split())
    except:
        code = ""
...
class GraphCodeBertInputFeatures(object):
    ...
    def __init__(self, input_tokens_1, input_ids_1, position_idx_1, dfg_to_code_1, dfg_to_dfg_1, input_tokens_2,
                 input_ids_2, position_idx_2, dfg_to_code_2, dfg_to_dfg_2, label, url1, url2):
...
def graphcodebert_convert_examples_to_features(item):
    # source
    shilihuacode, url1, url2, label, tokenizer, args, cache, url_to_code = item
    parser = parsers['java']
...
class CodeBertTextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train', block_size=512, pool=None):
        model_name='codebert'
        succeed_attacked_PATH = "../CodeTAE_gpt4oMini/CloneDetection/%s_test" % model_name

        index_set = []
        files = os.listdir(succeed_attacked_PATH)
        for file in files:
        ...
        with open('../our_defend/dataset/CloneDetection/ori/data.jsonl') as f:
            for line in f:
                line = line.strip()
                js = json.loads(line)
        ...
        with open(index_filename) as f:
            for idx, line in enumerate(f.readlines()):
                ...
                print("url1 = ",url1)
                print("url2 = ",url2)
                with open(os.path.join(succeed_attacked_PATH, file_name), 'r') as f:
                    shilihua_code = f.read()
                data.append((shilihua_code, url1, url2, label, tokenizer, args, cache, url_to_code))
...
class GraphCodeBertTextDataset(Dataset):
    def __init__(self, tokenizer, args, file_path='train'):
        ...
        succeed_attacked_PATH = "../CodeTAE_gpt4oMini/CloneDetection/shilihua_%s_test" % model_name
        index_set = []
        files = os.listdir(succeed_attacked_PATH)
        for file in files:
            index = file.split('_')[0]
        ...
        with open('../our_defend/dataset/CloneDetection/shilihua_ori/data.jsonl') as f:
