...
from run_parser import get_identifiers, get_identifiers_ori, get_example, get_example_batch, get_code_style, change_code_style
...
def get_embeddings(code, variables, tokenizer_mlm, codebert_mlm):
    ...
    _, _, code_tokens = get_identifiers(new_code, "c")
...
class Attacker:
    ...
    def attack(self, example, code):
        ...
        elif self.args.model_name == 'codet5' or self.args.model_name == 'codet5p':
            true_label = example[1].item()
        variable_names, function_names, code_tokens = get_identifiers(code, "c")
        ...
        for sub in random_subs:
            ...
            if self.args.model_name in ['codebert']:
                embeddings = np.load('../dataset/codebert_all_subs/%s_%s.npy' % (sub['label'], embeddings_index))
            elif self.args.model_name in ['graphcodebert']:
                embeddings = np.load('../dataset/graphcodebert_all_subs/%s_%s.npy' % (sub['label'], embeddings_index))
            elif self.args.model_name in ['codet5','codet5p']:
                embeddings = np.load('../dataset/codet5_all_subs/%s_%s.npy' % (sub['label'], embeddings_index))
        ...
        for temp in all_code_csc:
            ...
            if self.args.model_name == 'codebert':
                new_feature = codebert_convert_code_to_features(temp_code, self.tokenizer_tgt, example[1].item(),
                                                                self.args)
            elif self.args.model_name == 'graphcodebert':
                new_feature = graphcodebert_convert_code_to_features(temp_code, self.tokenizer_tgt, example[3].item(),
                                                                     self.args)
            elif self.args.model_name == 'codet5' or self.args.model_name == 'codet5p':
                new_feature = codet5_convert_code_to_features(temp_code, self.tokenizer_tgt, example[1].item(),
                                                              self.args)
        ...
        return -4, None, None  # add by lsr 1
        # 上面结构变换结束了
        ...
        for i in range(NUMBER_2):
            ...
            for j, tgt_word in enumerate(variable_names):
                #print("len(substituions[tgt_word] = ", len(substituions[tgt_word]))
