...
class CodeT5RobertaClassificationHead_twoContact(nn.Module):
    """Head for sentence-pair classification tasks, with concatenated features."""
...
class CodeBERTnoise(nn.Module):
    ...
    def forward(self, input_ids=None, labels=None, noise=None):
        ...
        outputs = self.encoder(inputs_embeds=inputs_embeddings, attention_mask=attn_mask, position_ids=position_ids,
...
class GraphCodeBERTnoise(nn.Module):
    ...
    def get_results(self, dataset, batch_size, new_infer=False):
        ...
        for batch in eval_dataloader:
            ...
            with torch.no_grad():
                lm_loss, logit, _, _ = self.forward(inputs_ids, attn_mask, position_idx, label,
                                                    noise=noise if new_infer else None)
                logits.append(logit.cpu().numpy())
        logits = np.concatenate(logits, 0)
        probs = logits
        pred_labels = []
...
class CodeT5noise(nn.Module):
    def __init__(self, encoder, config, tokenizer, args):
        ...
        self.encoder = encoder  # 可以是 T5EncoderModel 或 T5ForConditionalGeneration
        self.config = config
        self.tokenizer = tokenizer
        self.classifier = CodeT5RobertaClassificationHead(config)
        self.args = args
    ...
    def forward(self, input_ids=None, labels=None, noise=None):
        ...
        mask = input_ids.ne(padding_idx).int()
        incremental_indices = (torch.cumsum(mask, dim=1).type_as(mask)) * mask
        position_ids = incremental_indices.long() + padding_idx  # 虽然用不上，但保持你原来的结构
        ...
        # 给 decoder 一个 dummy 起始 token，满足 forward 要求
        decoder_input_ids = torch.full(
            (input_ids.size(0), 1),
            self.tokenizer.pad_token_id,  # 或 self.config.decoder_start_token_id 都行
            dtype=torch.long,
        ...
        # 拿 encoder 的输出
        sequence_outputs = outputs.encoder_last_hidden_state
        logits = self.classifier(sequence_outputs.mean(dim=1))  # pool 一下
        prob = F.softmax(logits, dim=-1)

        if labels is not None:
            loss_fct = CrossEntropyLoss()
            loss = loss_fct(logits, labels)
            return loss, prob, sequence_outputs, outputs.encoder_attentions
        else:
            return prob, logits
    ...
    def get_outputs(self, input_ids=None, labels=None, noise=None):
        ...
        outputs = self.encoder(input_ids=None, attention_mask=attention_mask,
                               labels=input_ids, decoder_attention_mask=attention_mask,
                               output_hidden_states=True, inputs_embeds=inputs_embeds)
    ...
    def get_results(self, dataset, batch_size, new_infer=False):
        ...
        for batch in eval_dataloader:
            ...
            noise = None
            if new_infer:
                # 拿 decoder_hidden_states, 自己写个 calculate_noise() 函数就行
                hidden_states = self.get_outputs(inputs)
                noise = calculate_noise(self, inputs, hidden_states, self.args)

            with torch.no_grad():
                lm_loss, logit, _, _ = self.forward(inputs, label, noise=noise)
                eval_loss += lm_loss.mean().item()
                logits.append(logit.cpu().numpy())
...
class CodeT5_twoContact(nn.Module):
    ...
    def forward(self, input_ids=None, labels=None, noise=None):
        ...
        # 取 <eos> 位，等价于 CodeT5 里的用法
        eos_mask = input_ids.eq(self.config.eos_token_id)
        if len(torch.unique(eos_mask.sum(1))) > 1:
            raise ValueError("All examples must have the same number of <eos> tokens.")

        rep1 = outputs1[eos_mask, :].view(outputs1.size(0), -1, outputs1.size(-1))[:, -1, :]  # shape: [batch, hidden]
