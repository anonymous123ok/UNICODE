...
MODEL_CLASSES = {
    ...
    'graphcodebert': (
    RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer, GraphCodeBERTnoise, GraphCodeBERT_twoContact),
...
def evaluate_model(args, model, tokenizer, eval_when_training=False, new_infer=False, contact=False, pool=None):
    ...
    if args.model_type == 'codebert':
        ...
        eval_dataset = CodeT5TextDataset(tokenizer, args, file_path=args.eval_data_file, pool=pool)
...
def test(args, model, tokenizer, eval_when_training=False, new_infer=False, pool=None):
    ...
    if args.model_type == 'codebert':
        ...
        eval_dataset = CodeT5TextDataset(tokenizer, args, file_path=args.test_data_file, pool=pool)
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=4)
    model.to(args.device)
    # multi-gpu evaluate
...
def main():
    ...
    parser.add_argument("--number_labels", default=4, type=int,
                        help="The model checkpoint for weights initialization.")
    ...
    parser.add_argument("--do_test", action='store_true',
                        help="Whether to run eval on the dev set.")
    parser.add_argument("--language_type", type=str, default="c",
    ...
    output_dir_single = '../model/{}/checkpoint-best-acc/{}_{}_model{}.bin'
    ...
    if args.do_train:
        if args.model_type == 'codebert':
            ...
            train_dataset = CodeT5TextDataset(model.tokenizer, args, args.train_data_file, pool=pool)
