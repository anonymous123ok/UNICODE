...
class RobertaClassificationHead_twoContact(nn.Module):
    ...
    def forward(self, features, **kwargs):
        if features.dim() == 3:
            x = features[:, 0, :]  # 取 <s> token 位置
        elif features.dim() == 2:
            x = features  # 直接是 pooled features
        else:
...
class CodeBERTnoise(nn.Module):
    ...
    def get_outputs(self, input_ids=None, labels=None, noise=None):
        ...
        opt = self.encoder(inputs_embeds=inputs_embeddings, attention_mask=input_ids.ne(1),
                           output_attentions=True)  # 我怀疑之前的报错可以通过修改这一行解决，先待定
        outputs = opt[0]
        return outputs
...
class GraphCodeBERT_twoContact(nn.Module):
    ...
    def forward(self, inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2, position_idx_2, attn_mask_2,
        ...
        outputs1 = self.model1.get_outputs(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2, position_idx_2, attn_mask_2,
         labels, noise)[:, 0, :]
        outputs2 = self.model2.get_outputs(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2, position_idx_2, attn_mask_2,
         labels, noise=None)[:, 0, :]
    ...
    def get_results(self, dataset, batch_size):
        ...
        for batch in eval_dataloader:
            (inputs_ids_1, position_idx_1, attn_mask_1,
             inputs_ids_2, position_idx_2, attn_mask_2,
             label) = [x.to("cuda") for x in batch]
            _, _, _, attentions = self.model1(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
                                              position_idx_2, attn_mask_2, label)
            bs, l = inputs_ids_1.size()
            inputs_ids = torch.cat((inputs_ids_1.unsqueeze(1), inputs_ids_2.unsqueeze(1)), 1).view(bs * 2, l)
            ...
            with torch.no_grad():
                ...
                lm_loss, logit = self.forward(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
                                              position_idx_2, attn_mask_2, label, noise)
                # 调用这个模型. 重写了反前向传播模型.
                eval_loss += lm_loss.mean().item()
                logits.append(logit.cpu().numpy())
            nb_eval_steps += 1
