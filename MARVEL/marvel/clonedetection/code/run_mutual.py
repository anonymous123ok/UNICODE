...
# from model import (CodeBERTnoise, GraphCodeBERTnoise, UniXCodernoise, CodeT5noise,
#                    CodeBERT_twoContact, GraphCodeBERT_twoContact, UniXCoder_twoContact, CodeT5_twoContact)
from model import (CodeBERTnoise,CodeBERT_twoContact,CodeT5noise,CodeT5_twoContact,GraphCodeBERTnoise,GraphCodeBERT_twoContact)
cpu_cont = 16
...
#     'codet5': (T5Config, T5ForConditionalGeneration, RobertaTokenizer, CodeT5noise, CodeT5_twoContact),
#     'codet5p': (T5Config, T5ForConditionalGeneration, RobertaTokenizer, CodeT5noise, CodeT5_twoContact)
# }
MODEL_CLASSES = {
    'codebert': (RobertaConfig, RobertaModel, RobertaTokenizer, CodeBERTnoise, CodeBERT_twoContact),
    'graphcodebert': (RobertaConfig, RobertaForSequenceClassification, RobertaTokenizer, GraphCodeBERTnoise, GraphCodeBERT_twoContact),
...
def graphcodebert_single_train(model1, model2, noise, inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
                                                   position_idx_2, attn_mask_2, labels, args, former_noise=True):
    # 互相学习, model1向model2学习
    if former_noise:
        loss1, c_prob1, c_outputs1, attentions1 = model1(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
               position_idx_2, attn_mask_2, labels, noise=noise)
        loss2, c_prob2, c_outputs2, attentions2 = model2(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
               position_idx_2, attn_mask_2, labels)
    else:
        loss1, c_prob1, c_outputs1, attentions1 = model1(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
               position_idx_2, attn_mask_2, labels)
        loss2, c_prob2, c_outputs2, attentions2 = model2(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
...
def calculate_noise(model, inputs_ids, attentions, args):
    ...
    inputs_ids = inputs_ids.view(-1,args.block_size) # add by me
    ...
    if args.model_type == 'codebert':
        ...
        embeddings = model.encoder.encoder.embed_tokens(inputs_ids)
        ...
        for identifier in identifiers_list:
            ...
            for layer in attentions[batch_id]:
                ...
                for idn in token_index_map_list[batch_id][identifier]:
                    # if idn >= layer.size(0):
                    #     continue
                    if args.model_type == 'codet5' or args.model_type == 'codet5p':
                        temp_val += layer[:, idn, idn].mean().item()
                    else:
    ...
    for idx in range(args.epochs):
        ...
        for step, batch in enumerate(bar):
            if args.model_type == 'codebert':
                ...
                model1.train()
                model2.train()
                _, _, _, attentions = model1(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
                                                   position_idx_2, attn_mask_2, labels)
            ...
            if args.model_type == 'codebert' or args.model_type == 'unixcoder' or args.model_type == 'codet5' or args.model_type == 'codet5p':
                attn_mask = None
                position_idx = None
            ...
            for step_adv in range(args.max_adv_step):
                ...
                if args.model_type == 'graphcodebert':
                    train_loss1 = graphcodebert_single_train(model1, model2, noise, inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
                                                   position_idx_2, attn_mask_2,  labels, args,former_noise=True)
                else:
                    # model1 加噪, model1 向 model2 学习
                    train_loss1 = single_train(model1, model2, noise, inputs_ids, attn_mask, position_idx, labels, args,
                                           former_noise=True)
            ...
            if args.model_type == 'graphcodebert':
                train_loss2 = graphcodebert_single_train(model2, model1, noise, inputs_ids_1, position_idx_1, attn_mask_1,
                                                     inputs_ids_2,position_idx_2, attn_mask_2, labels, args, former_noise=True)
            else:
                train_loss2 = single_train(model2, model1, noise, inputs_ids, attn_mask, position_idx, labels, args,
                                       former_noise=False)
            ...
            if global_step % args.save_steps == 0:
                ...
                if results1['eval_f1'] > best_f1_1:
                    best_f1_1 = results1['eval_f1']
                    logger.info("  " + "*" * 20)
                    logger.info(" Model1 Best F1:%s", round(best_f1_1, 4))
                    logger.info("  " + "*" * 20)
                ...
                if results2['eval_f1'] > best_f1_2:
                    best_f1_2 = results2['eval_f1']
                    logger.info("  " + "*" * 20)
                    logger.info(" Model2 Best F1:%s", round(best_f1_2, 4))
                    logger.info("  " + "*" * 20)
...
def evaluate_model(args, model, tokenizer, eval_when_training=False, new_infer=False, contact=False, pool=None):
    ...
    if args.model_type == 'codebert':
        ...
        eval_dataset = CodeT5TextDataset(tokenizer, args, file_path=args.eval_data_file)
    ...
    for batch in eval_dataloader:
        if args.model_type == 'codebert' or args.model_type == 'codet5' or args.model_type == 'codet5p':
            inputs_ids = batch[0].to(args.device)
            ...
            (inputs_ids_1, position_idx_1, attn_mask_1,
             inputs_ids_2, position_idx_2, attn_mask_2,
             labels) = [x.to("cuda") for x in batch]
            # inputs_ids = batch[0].to(args.device)
            # attn_mask = batch[1].to(args.device)
            # position_idx = batch[2].to(args.device)
            ...
            if new_infer:
                if contact:
                    ...
                    _, _, _, attentions = model.model1(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
                                                 position_idx_2, attn_mask_2, labels)
                    bs, l = inputs_ids_1.size()
                    inputs_ids = torch.cat((inputs_ids_1.unsqueeze(1), inputs_ids_2.unsqueeze(1)), 1).view(bs * 2, l)
                    # position_idx = torch.cat((position_idx_1.unsqueeze(1), position_idx_2.unsqueeze(1)), 1).view(bs * 2,l)
                    ...
                    _, _, _, attentions = model(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
                                                 position_idx_2, attn_mask_2, labels)
                    bs, l = inputs_ids_1.size()
                    inputs_ids = torch.cat((inputs_ids_1.unsqueeze(1), inputs_ids_2.unsqueeze(1)), 1).view(bs * 2, l)
                    noise = calculate_noise(model, inputs_ids, attentions, args)
            with torch.no_grad():
                prob, c_logit = model(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
                 position_idx_2, attn_mask_2,noise=noise if new_infer else None)
...
def test(args, model, tokenizer, eval_when_training=False, new_infer=False, pool=None):
    # build dataloader
    if args.model_type == 'codebert':
        eval_dataset = CodeBertTextDataset(tokenizer, args, file_path=args.test_data_file)
    elif args.model_type == 'graphcodebert':
        eval_dataset = GraphCodeBertTextDataset(tokenizer, args, file_path=args.test_data_file)
    elif args.model_type == 'unixcoder':
        eval_dataset = UniXCoderTextDataset(tokenizer, args, file_path=args.test_data_file)
    elif args.model_type == 'codet5' or args.model_type == 'codet5p':
        eval_dataset = CodeT5TextDataset(tokenizer, args, file_path=args.test_data_file)
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=args.eval_batch_size, num_workers=4)
    model.to(args.device)
    # multi-gpu evaluate
    ...
    print("F1 in test set:", result['test_f1'])
...
def train_contact(args, train_dataset, tokenizer, model_contact, pool=None):
    ...
    optimizer = AdamW(filter(lambda p: p.requires_grad, model_contact.parameters()), lr=args.learning_rate,
        ...
        for step, batch in enumerate(bar):
            if args.model_type == 'codebert' or args.model_type == 'codet5' or args.model_type == 'codet5p':
                ...
                (inputs_ids_1, position_idx_1, attn_mask_1,
                 ...
                 labels) = [x.to(args.device) for x in batch]
                model_contact.train()
                _, _, _, attentions = model_contact.model1(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
                                                   position_idx_2, attn_mask_2, labels)
                bs, l = inputs_ids_1.size()
                ...
                loss, probs = model_contact(inputs_ids_1, position_idx_1, attn_mask_1, inputs_ids_2,
                                          position_idx_2, attn_mask_2, labels, noise=noise)
            ...
            if global_step % args.save_steps == 0:
                results1 = evaluate_model(args, model_contact, tokenizer, eval_when_training=True, new_infer=True,
                                          contact=True, pool=pool)
                # Save model checkpoint
                if results1['eval_f1'] >= best_f1:
                    best_f1 = results1['eval_f1']
                    logger.info("  " + "*" * 20)
                    logger.info(" Model Best F1:%s", round(best_f1, 4))
                    logger.info("  " + "*" * 20)
...
def main():
    ...
    parser.add_argument("--number_labels", default=2, type=int,
                        help="The model checkpoint for weights initialization.")
    ...
    parser.add_argument("--do_test", action='store_true',
                        help="Whether to run eval on the dev set.")
    parser.add_argument("--language_type", type=str, default="java",
    ...
    args.datajsonl = '../dataset/data.jsonl'
    ...
    if args.do_train:
        if args.model_type == 'codebert':
            train_dataset = CodeBertTextDataset(model.tokenizer, args, args.train_data_file)
        elif args.model_type == 'graphcodebert':
            train_dataset = GraphCodeBertTextDataset(model.tokenizer, args, args.train_data_file)
        elif args.model_type == 'unixcoder':
            train_dataset = UniXCoderTextDataset(model.tokenizer, args, args.train_data_file)
        elif args.model_type == 'codet5' or args.model_type == 'codet5p':
            train_dataset = CodeT5TextDataset(model.tokenizer, args, args.train_data_file)
    ...
    if args.do_eval:
        model_contact = Model_Contact(config, model1, model2, args)
        checkpoint_prefix = 'checkpoint-best-f1/{}_{}_model.bin'.format(args.model_type, args.save_name)
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
    ...
    if args.do_test:
        ...
        checkpoint_prefix = 'checkpoint-best-f1/{}_{}_model.bin'.format(args.model_type, args.save_name)
        output_dir = os.path.join(args.output_dir, '{}'.format(checkpoint_prefix))
        model_contact.load_state_dict(
            torch.load(output_dir, map_location=torch.device("cuda" if torch.cuda.is_available() else "cpu")),
            strict=False)
        model_contact.to(args.device)
