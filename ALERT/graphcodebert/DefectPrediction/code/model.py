...
class Model(nn.Module):
    ...
    def get_results(self, dataset, batch_size):
        ...
        eval_dataloader = DataLoader(dataset, sampler=eval_sampler, batch_size=batch_size, pin_memory=False)

        self.eval()
        logits = []
        labels = []
        for batch in eval_dataloader:
            inputs_ids = batch[0].to("cpu")
            attn_mask = batch[1].to("cpu")
            position_idx = batch[2].to("cpu")
            label = batch[3].to("cpu")
            with torch.no_grad():
