    ...
    def mcmc_random(self, tokenizer, code=None, _label=None, _n_candi=30,
        ...
        for iteration in range(1, 1+_max_iter):
            ...
            if res['status'].lower() in ['s', 'a']:
                ...
                uid_value = uid.pop(res['old_uid'])
                uid[res['new_uid']] = uid_value

                print("After updating uid:")
                print(f"Old UID: {res['old_uid']}, New UID: {res['new_uid']}")
                print("Updated UID Dictionary:", uid)
        ...
        for uid_ in old_uids.keys():
            last_uid = old_uids[uid_][-1]
            if last_uid not in uid:
                print(f"KeyError: Last UID '{last_uid}' is not in uid.")
                print("uid:")
                print(uid)
            else:
                nb_changed_pos += len(uid[last_uid])
