...
from python_parser.run_parser import get_identifiers, remove_comments_and_docstrings, get_function_name
...
class Args:
    def __init__(self):
        self.eval_data_file = '/home/yanmeng/huangli/Attack/dataset/Clone-detection/test_sampled.txt'
        self.base_model = '/home/yanmeng/huangli/.cache/huggingface/hub/models--microsoft--codebert-base-mlm'
        self.store_path = './test_subs_clone_all.jsonl'
        self.block_size = 512
        self.index = [0, 4000]
...
def main():
    ...
    with open('/home/yanmeng/huangli/Attack/dataset/Clone-detection/data.jsonl') as f:
        for line in f:
            line=line.strip()
            js=json.loads(line)
    ...
    with open(args.store_path, "a") as wf:
        for item in tqdm(eval_data):
            try:
                ...
                function_name = get_function_name(remove_comments_and_docstrings(item["code1"], "java"), "java")
            except:
                identifiers, code_tokens = get_identifiers(item["code1"], "java")
                function_name = get_function_name(item["code1"], "java")
            processed_code = " ".join(code_tokens)

            identifiers = function_name + identifiers
