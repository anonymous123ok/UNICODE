...
def convert_examples_to_features(code1_tokens,code2_tokens,label,url1,url2,tokenizer,args,cache):
    ...
    # print(code1_ids)
    # print(len(code1_ids)) #510

    # print(source_ids)
    # print(len(source_ids)) #1020
            ...
            with open(index_filename) as f:
                for line in f:
                    ...
                    if label=='0':
                        ...
                        label=1
                    # print("len of url_to_code: ",len(url_to_code)) #9126
                    # print(url_to_code)
...
def ASR(args, model, tokenizer):
    ...
    print('length of eval_dataloader: {}, length of adv_codes: {}'.format(len(eval_dataloader), len(adv_codes)))   # test
    ...
    for i, (example) in enumerate(eval_dataloader):
        input_id = example[0].to(args.device)
        label = example[1].to(args.device)
        # print('input_id.shape: ', input_id.shape) #torch.Size([1, 1020])
        with torch.no_grad():
            lm_loss, logit = model(input_id,label)
        ...
        if pred_label == label:
            ...
            adv_inputs_id = convert_advs_to_features(adv_codes[i], code2, tokenizer, args)
            adv_inputs_id = torch.tensor(adv_inputs_id).unsqueeze(0).to(args.device)
            # print('adv_inputs_id.shape: ', adv_inputs_id.shape) #torch.Size([1, 1020])
            with torch.no_grad():
            ...
            if adv_pred_label != label:
                # print('label: ', label)
                if label == 1 and adv_pred_label==0:
                    num_1_0 += 1
                attack_success_num += 1

    # print('num_0: ', num_0)
    # print('num_1: ', num_1)
    # print('num_1_0: ', num_1_0)
...
def similarity(args, model, tokenizer):
    ...
    list_distance = []
    #pre-trained encoder
    model_sub = Model_sub(RobertaModel.from_pretrained('microsoft/codebert-base'), args=args).to(args.device)
    ...
    for i, (example) in enumerate(eval_dataloader):
        ...
        # print('input_id.shape: ', input_id.shape) #torch.Size([1, 1020])
        with torch.no_grad():
            lm_loss, logit, fine_tuned_embedding = model(input_id,label)
        logit = logit.cpu().numpy()
        label = label.cpu().numpy()[0]
        pred_label = 0 if (logit[:,0]>0.5)[0] else 1
        # print(label)
        ...
        pre_trained_embedding = model_sub(input_id) #torch.Size([2, 510, 768])
...
def main():
    ...
    parser.add_argument("--adv_data_file", default=None, type=str,
                        help="An optional input adversarial data file to evaluate the perplexity on (a text file).")
    ...
    parser.add_argument("--do_asr", action='store_true',
                        help="Whether to run eval on the adv set.")    
    parser.add_argument("--do_sim", action='store_true',
                        help="Whether to run eval on the adv set.")
    parser.add_argument("--train_p", type=float, default=1.0,
                        help="Ratio of tokens to mask for masked language modeling loss")
    ...
    if args.do_train:
        ...
        train_dataset = random.sample(list(train_dataset), round(len(train_dataset)*args.train_p))
        # print(len(train_dataset))
        if args.local_rank == 0:
