...
def main():
    ...
    config = config_class.from_pretrained('../CodeTAE/codet5-base')
    config.num_labels=2
    tokenizer = RobertaTokenizer.from_pretrained('CODA_new/CodeTAE/codet5-base')

    if args.block_size <= 0:
        args.block_size = tokenizer.max_len_single_sentence  # Our input block size will be the max possible for the model
    args.block_size = min(args.block_size, tokenizer.max_len_single_sentence)
    if args.model_name_or_path:
        model_tgt = model_class.from_pretrained('CODA_new/CodeTAE/codet5-base')
    else:
    ...
    try:
        model_sub = T5ForConditionalGeneration.from_pretrained("/CODA_new/CodeTAE/codet5-base").to(args.device)
    except:
        model_sub = T5ForConditionalGeneration.from_pretrained('/CODA_new/CodeTAE/codet5-base').to(args.device)
    ...
    attack_type_list = ['identifier_renaming', 'insert_unused_variable', 'insert_empty_print', 'insert_empty_if',
                        'insert_empty_while']
        ...
        try:
            if 'identifier_renaming' in attack_type_list:
                ...
                code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.greedy_attack(
                    example, substitute, code_pair)
                # substitute_2
                if is_success == -1:
                    args.num_of_changes = 2
                    code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.greedy_attack(
                        example, substitute_2, code)
                if is_success == 1:
            ...
            if 'insert_unused_variable' in attack_type_list:
                ...
                if is_success == -1:
                    ...
                    code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.insert_attack(
                        example, substitute, code, adv=ours_code)
                    if is_success == -1:
                        args.num_of_changes = 6
                        code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.insert_attack(
                            example, substitute_2, code, adv=ours_code)
            ...
            if 'insert_empty_if' in attack_type_list:
                ...
                if is_success == -1:
                    ...
                    code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.nonreachable_if_attack(
                        example, substitute, code, adv=ours_insert_print_code)
                    if is_success == -1:
                        args.num_of_changes = 15
                        code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.nonreachable_if_attack(
                            example, substitute_2, code, adv=ours_insert_print_code)

            if 'insert_empty_while' in attack_type_list:
                ours_insert_print_if_code = adv_code
                if is_success == -1:
                    args.num_of_changes = 15
                    code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.nonreachable_while_attack(
                        example, substitute, code, adv=ours_insert_print_if_code)
                    if is_success == -1:
                        args.num_of_changes = 15
                        code, prog_length, adv_code, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.nonreachable_while_attack(
                            example, substitute_2, code, adv=ours_insert_print_if_code)
        except KeyError:
            continue
...
''' 说实话我没看懂这个作者这里是要干什么，好像这段代码是另一个人写的，写的很烂
    for index, example in enumerate(eval_dataset):
        rename_flag = 0
        if index + args.start_index <= max(a_set) or index + args.start_index <= max(b_set):
            print("continue")
        ...
        if is_success == -1:
            ...
            if is_success == -1:
                code, prog_length, adv_code_after_if, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.nonreachable_if_attack(example,  subs_2, code_pair)
        ...
        if is_success == -1:
            ...
            if is_success == -1:
                code, prog_length, adv_code_after_while, true_label, orig_label, temp_label, is_success, variable_names, names_to_importance_score, nb_changed_var, nb_changed_pos, replaced_words = attacker.nonreachable_while_attack(example,  subs_2, code_pair)
