            ...
            if (step + 1) % args.gradient_accumulation_steps == 0:
                ...
                if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:
                    ...
                    if results['eval_accuracy']>=best_f1:
                        ...
                        output_dir = os.path.join(output_dir, 'model{}.bin'.format(ft))
...
def ASR(args, model, tokenizer):
    ...
    # adv_dataloader = DataLoader(adv_dataset, sampler=adv_sampler, batch_size=1 ,num_workers=4, pin_memory=True)
    ...
    for i, (example_eval) in enumerate(eval_dataloader):
        eval_input_id = example_eval[0].to(args.device)
        eval_label = example_eval[1].to(args.device)
        with torch.no_grad():
            eval_lm_loss, eval_logit = model(eval_input_id, eval_label)
        ...
        if np.argmax(eval_logit) == eval_label:
            total_num += 1
            adv_input_id = torch.tensor(convert_examples_to_features(adv_codes[i], eval_label, tokenizer, args).input_ids).unsqueeze(0).to(args.device)
            # adv_input_id = example_adv[0].to(args.device)
            with torch.no_grad():
                adv_logit = model(adv_input_id)
...
def ASR2(args, model, tokenizer):
    ...
    for i, (example_eval) in enumerate(eval_dataloader):
        ...
        if np.argmax(eval_logit) == eval_label:
            infer_success_num += 1
    suc = infer_success_num / total_num
    result = {
        "successful acc ": suc,
        "total_num": total_num,
        "success_num": infer_success_num
    }
    logger.info("***** Defend results *****")
    for key in sorted(result.keys()):
        logger.info("  %s = %s", key, str(round(result[key], 4)))
...
def similarity(args, model, tokenizer, config):
    ...
    list_distance = []
    try:
        model_sub = CodeT5_sub(T5ForConditionalGeneration.from_pretrained("Salesforce/codet5-small"),config,tokenizer,args).to(args.device)
    except:
        model_sub = CodeT5_sub(T5ForConditionalGeneration.from_pretrained('../cache/Salesforce/codet5-small/'),config,tokenizer,args).to(args.device)
    distance = nn.PairwiseDistance(p=2)
    for i, (example_eval) in enumerate(eval_dataloader):
        eval_input_id = example_eval[0].to(args.device)
        eval_label = example_eval[1].to(args.device)
        with torch.no_grad():
            eval_lm_loss, eval_logit, fine_tuned_embedding = model(eval_input_id, eval_label)
        ...
        #样本原本分类正确则继续
        if np.argmax(eval_logit) == eval_label:
            pre_trained_embedding = model_sub(eval_input_id)
...
def main():
    ...
    try:
        ...
        config = config_class.from_pretrained('/workspace/CODA_new/CodeTAE/codet5-base')
        config.num_labels=args.number_labels
        tokenizer = RobertaTokenizer.from_pretrained('/workspace/CODA_new/CodeTAE/codet5-base')
    except:
        config_class, model_class, tokenizer_class = MODEL_CLASSES[args.model_type]
    ...
    if args.model_name_or_path:
        try:
            encoder = model_class.from_pretrained('/workspace/CODA_new/CodeTAE/codet5-base')
        except:
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ft = '_ft' if args.ft else ''
        # checkpoint_prefix = 'checkpoint-best-f1/model_at{}.bin'.format(ft)
