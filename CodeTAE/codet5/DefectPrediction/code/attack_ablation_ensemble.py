...
def main():
    ...
    if args.model_name_or_path:
        model_tgt = model_class.from_pretrained('/workspace/CODA_new/CodeTAE/codet5-base')
    else:
    ...
    try:
        model_sub = T5ForConditionalGeneration.from_pretrained("/workspace/CODA_new/CodeTAE/codet5-base").to(args.device)
    except:
        model_sub = T5ForConditionalGeneration.from_pretrained('/workspace/CODA_new/CodeTAE/codet5-base').to(args.device)
    ...
    for index, (example_eval) in enumerate(eval_dataset):
        rename_flag = 0
        if index < args.start_index or index >= args.start_index + 25:
            # print("continue")
            continue
