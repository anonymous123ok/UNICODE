...
class CrossDomainAttacker():
    ...
    def loss(self, embedding_a, embedding_b):
        ...
        '''        
        return nn.MSELoss().to(self.args.device)(embedding_a.to(self.args.device), embedding_b.to(self.args.device)).item()
    ...
    def get_importance_score(self, code, words_list: list, sub_words: list, variable_names: list, model_type='classification', target_example=None):
        ...
        for index, tokens in enumerate([words_list] + masked_token_list):
            ...
            embedding = self.convert_code_to_embedding(new_code).detach().cpu()
    ...
    def greedy_attack(self, example, code, subs, target_example=None):
        ...
        adv_example = example
        adv_code = code
        orig_embeddings = self.convert_code_to_embedding(orig_code)
        ...
        adv_embeddings = self.convert_code_to_embedding(adv_code).detach().cpu()
        for k, _ in subs.items():
            if k in subs[k]:
                subs[k].remove(k)
        ...
        for name_and_score in sorted_list_of_names:
            ...
            for substitute in all_substitutes:
                ...
                replace_embeddings_list.append(self.convert_code_to_embedding(temp_code).detach().cpu())
        ...
        for name_and_score in sorted_list_of_names:
            ...
            for current_beam in beam_search_candidates:
                ...
                for substitute in all_substitutes:
                    ...
                    # 需要将几个位置都替换成sustitue_
                    temp_code = get_example(current_beam['code'], tgt_word, substitute, "c")
    ...
    def nonreachable_if_attack(self, example, code, subs, target_example=None):
        ...
        for k, v in subs.items():
            subs_for_insert = v
            break
        subs_for_insert = subs_for_insert[:min(20, len(subs_for_insert))]

        inserted_num = 0
        print('inserting...')
        print('length of sorted_id: {}'.format(len(sorted_id)))
        for i, inserted_id in enumerate(sorted_id):
            print('i = {}...'.format(i))   # test
            update_flag = False
            for sub in subs_for_insert:
