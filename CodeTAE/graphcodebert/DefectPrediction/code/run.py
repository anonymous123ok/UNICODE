...
for lang in dfg_function:
    LANGUAGE = Language('/workspace/CODA_new/CODA/python_parser/parser_folder/my-languages.so', lang)
    parser = Parser()
...
def convert_advs_to_features(code, tokenizer,args):
    #source
    # code=' '.join(js['func'].split())
    parser = parsers["c"]
    code_tokens,dfg = extract_dataflow(code, parser, "c")
    ...
    return torch.tensor(source_ids), torch.tensor(attn_mask), torch.tensor(position_idx)
...
def ASR(args, model, tokenizer):
    eval_dataset = TextDataset(tokenizer, args, file_path=args.eval_data_file)
    eval_sampler = SequentialSampler(eval_dataset)
    eval_dataloader = DataLoader(eval_dataset, sampler=eval_sampler, batch_size=1,num_workers=0)
    ...
    for i, (example_eval) in enumerate(eval_dataloader):
        ...
        eval_attn_mask = example_eval[1].to(args.device) 
        eval_position_idx = example_eval[2].to(args.device) 
        eval_label=example_eval[3].to(args.device) 
        with torch.no_grad():
        ...
        if np.argmax(eval_logit) == eval_label:
            total_num += 1
            adv_inputs_id, adv_attn_mask, adv_position_idx = convert_advs_to_features(adv_codes[i], tokenizer,args)
            adv_inputs_id = adv_inputs_id.unsqueeze(0).to(args.device)
            adv_attn_mask = adv_attn_mask.unsqueeze(0).to(args.device)
            adv_position_idx = adv_position_idx.unsqueeze(0).to(args.device)
            # adv_inputs_id = example_adv[0].to(args.device)
            # adv_attn_mask = example_adv[1].to(args.device) 
            # adv_position_idx = example_adv[2].to(args.device) 
            # adv_label=example_adv[3].to(args.device) 
            with torch.no_grad():
                adv_logit = model(adv_inputs_id,adv_attn_mask,adv_position_idx)
...
def similarity(args, model, tokenizer):
    ...
    for i, (example_eval) in enumerate(eval_dataloader):
        ...
        eval_label=example_eval[3].to(args.device) 
        with torch.no_grad():
            eval_lm_loss, eval_logit, fine_tuned_embedding = model(eval_inputs_id,eval_attn_mask,eval_position_idx,eval_label)
        ...
        #样本原本分类正确则继续
        if np.argmax(eval_logit) == eval_label:
            pre_trained_embedding = model_sub(eval_inputs_id,eval_attn_mask,eval_position_idx)
            # print('pre_trained_embedding.shape: ',pre_trained_embedding.shape) #torch.Size([1, 512, 768])
...
def main():
    ...
    parser.add_argument("--adv_data_file", default=None, type=str,
                        help="An optional input attack data file to evaluate the perplexity on (a text file).")
