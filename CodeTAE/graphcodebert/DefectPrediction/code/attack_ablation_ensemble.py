...
def main():
    ...
    try:
        ...
        config = config_class.from_pretrained('/workspace/CODA_new/CODA/microsoft/graphcodebert-base/config.json')
    ...
    try:
        ...
        tokenizer = tokenizer_class.from_pretrained('/workspace/CODA_new/CODA/microsoft/graphcodebert-base/')
    ...
    if args.model_name_or_path:
        try:
            ...
            model_tgt = model_class.from_pretrained('/workspace/CODA_new/CODA/microsoft/graphcodebert-base')
    else:
    ...
    model_sub = Model_sub(RobertaForSequenceClassification.from_pretrained("/workspace/CODA_new/CODA/microsoft/graphcodebert-base")).to(args.device)


    ## Load CodeBERT (MLM) model
    try:
        codebert_mlm = RobertaForMaskedLM.from_pretrained("/workspace/CODA_new/CODA/microsoft/graphcodebert-base")
    except:
        codebert_mlm = RobertaForMaskedLM.from_pretrained("/workspace/CODA_new/CODA/microsoft/graphcodebert-base")
    try:
        tokenizer_mlm = RobertaTokenizer.from_pretrained("/workspace/CODA_new/CODA/microsoft/graphcodebert-base")
    except:
        tokenizer_mlm = RobertaTokenizer.from_pretrained("/workspace/CODA_new/CODA/microsoft/graphcodebert-base")
    ...
    for index, (example_eval) in enumerate(eval_dataset):
        rename_flag = 0
        if index < args.start_index or index >= args.start_index + 500:
            # print("continue")
            continue
        if index <= max(a_set) or index <= max(b_set):
            print("continue")
            continue
        ...
        # print(content)
        code_tokens = get_code_tokens(content, 'c')
        ...
        if is_success == 1:
            ...
            if rename_flag == 1:
                ...
                with open(attacked_code_path2 + str(index) + "_" + str(true_label) + ".txt",
                          "w") as file:
                    file.write(new_content)
